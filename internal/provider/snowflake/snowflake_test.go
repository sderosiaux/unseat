...
func TestProviderName(t *testing.T) {
	p := New("test-token", "myaccount")
	assert.Equal(t, "snowflake", p.Name())
}
...
func TestListUsers(t *testing.T) {
	...
	assert.Equal(t, "SF1", users[0].ProviderID)
...
func TestListUsersPagination(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if callCount == 1 {
			...
			json.NewEncoder(w).Encode(scimListResponse{
				Resources: []scimUser{
					{ID: "SF1", DisplayName: "User 1", Emails: []scimEmail{{Value: "u1@co.com", Primary: true}}, Active: true},
					{ID: "SF2", DisplayName: "User 2", Emails: []scimEmail{{Value: "u2@co.com", Primary: true}}, Active: true},
				},
				TotalResults: 3,
				ItemsPerPage: 2,
				StartIndex:   1,
			...
			json.NewEncoder(w).Encode(scimListResponse{
				Resources: []scimUser{
					{ID: "SF3", DisplayName: "User 3", Emails: []scimEmail{{Value: "u3@co.com", Primary: true}}, Active: true},
				},
				TotalResults: 3,
				ItemsPerPage: 2,
				StartIndex:   3,
...
func TestRemoveUser(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodGet {
			json.NewEncoder(w).Encode(scimListResponse{
				Resources: []scimUser{
					{ID: "SF1", DisplayName: "Alice", Emails: []scimEmail{{Value: "alice@co.com", Primary: true}}, Active: true},
				},
				TotalResults: 1,
				ItemsPerPage: 100,
				StartIndex:   1,
			...
			assert.Equal(t, "/scim/v2/Users/SF1", r.URL.Path)
			assert.Equal(t, "Bearer test-token", r.Header.Get("Authorization"))
...
func TestAPIError(t *testing.T) {
	...
	p := New("bad-token", "myaccount").WithBaseURL(server.URL)
