# saas-watcher Implementation Plan
...
**Design doc:** `docs/plans/2026-03-02-saas-watcher-design.md`
...
**Files:**
- Create: `go.mod`
- Create: `cmd/saas-watcher/main.go`
- Create: `Makefile`
- Create: `.gitignore`

**Step 1: Initialize Go module**

Run: `go mod init github.com/sderosiaux/saas-watcher`
...
# Makefile
BINARY := saas-watcher
PKG := github.com/sderosiaux/saas-watcher
VERSION := 0.1.0
...
Run: `go get github.com/spf13/cobra && go build ./cmd/saas-watcher`
Expected: binary compiles without errors

**Step 7: Run it**

Run: `go run ./cmd/saas-watcher --help`
...
- Create: `saas-watcher.example.yaml`
...
```bash
git add config/ saas-watcher.example.yaml
...
Run: `go build ./cmd/saas-watcher && ./bin/saas-watcher audit --help`
...
Run: `go build ./cmd/saas-watcher && ./bin/saas-watcher providers --help`
...
Run: `go build ./cmd/saas-watcher && ./bin/saas-watcher history --help`
...
Run: `go build -o bin/saas-watcher ./cmd/saas-watcher && ./bin/saas-watcher --help`
...
func (p *Provider) AddUser(ctx context.Context, email, role string) error {
	...
	// This is typically handled by Google Workspace admin, not saas-watcher.
...
Run: `go get github.com/go-chi/chi/v5 && go build -o bin/saas-watcher ./cmd/saas-watcher`
Expected: compiles
...
func New(s store.Store, cfg *config.Config) *MCPServer {
	...
	m.server = mcpsdk.NewServer(
		&mcpsdk.Implementation{Name: "saas-watcher", Version: "0.1.0"},
		nil,
	)
	m.registerTools()
	return m
...
Run: `go get github.com/modelcontextprotocol/go-sdk@v1.2.0 && go build -o bin/saas-watcher ./cmd/saas-watcher`
Expected: compiles
...
Run: `go build -o bin/saas-watcher ./cmd/saas-watcher`
Expected: compiles

**Step 3: Verify all CLI commands**

Run: `./bin/saas-watcher --help`
Expected: shows audit, sync, providers, history, serve, mcp

**Step 4: Run with example config**

Run: `cp saas-watcher.example.yaml saas-watcher.yaml && ./bin/saas-watcher providers list --json`
